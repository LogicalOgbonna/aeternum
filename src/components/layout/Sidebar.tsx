...
export function Sidebar({ isOpen = false, onClose }: SidebarProps) {
  ...
  const navigation = useMemo(() => {
    // Filter out Lending from mainNav if feature is off (in case it's added there)
    const baseNav = mainNav.filter(item => item.href !== '/lending');
    return showLendingFeature
      ? [...baseNav, { name: 'Lending', href: '/lending', icon: Banknote }]
      : baseNav;
  }, [showLendingFeature]);
      ...
      >
        ...
        <div className='p-6 border-b border-(--color-border) flex items-center justify-between'>
          ...
          >
            ...
            <div>
              <h1 className='font-bold text-lg text-(--color-text)'>
                Aeternum
              </h1>
